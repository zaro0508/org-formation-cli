...
export class UpdateNunjucksBuildTaskProvider implements IBuildTaskProvider<IUpdateNunjuckTaskConfiguration> {
    public type = 'update-nunjucks';

    createTask(config: IUpdateNunjuckTaskConfiguration, command: IPerformTasksCommandArgs): IUpdateNunjucksBuildTask {

        Validator.ValidateUpdateStacksTask(config, config.LogicalName);

        const task: IUpdateNunjucksBuildTask = {
            type: config.Type,
            ...
            perform: async (): Promise<void> => {
                const updateStacksCommand = UpdateNunjucksBuildTaskProvider.createUpdateStacksCommandArgs(config, command);
    ...
    createTaskForValidation(config: IUpdateNunjuckTaskConfiguration, command: IPerformTasksCommandArgs): IUpdateNunjucksBuildTask {
        return {
            type: config.Type,
            name: config.LogicalName,
            childTasks: [],
    ...
    createTaskForPrint(config: IUpdateNunjuckTaskConfiguration, command: IPerformTasksCommandArgs): IUpdateNunjucksBuildTask {
        return {
            type: config.Type,
            name: config.LogicalName,
            childTasks: [],
    ...
    static createUpdateStacksCommandArgs(config: IUpdateNunjuckTaskConfiguration, command: IPerformTasksCommandArgs): IUpdateStacksCommandArgs {
        ...
        if (config.OrganizationFile) {
            ConsoleUtil.LogWarning(`task ${this.name} specifies an attribute OrganizationFile which is ignored. The Template specified in the update-organization task is always used as OrganizationFile for update-nunjucks tasks`);
        }
...
export interface IUpdateNunjucksBuildTask extends IBuildTask {
    StackName: string;
}

export interface IUpdateNunjuckTaskConfiguration extends IBuildTaskConfiguration {
    Template: string;
    StackName: string;
