...
export class UpdateStacksCommand extends BaseCliCommand<IUpdateStacksCommandArgs> {
    ...
    public async performCommand(command: IUpdateStacksCommandArgs): Promise<void> {
        ...
        const cfnBinder = new CloudFormationBinder(stackName, template, state, parameters, data,command.forceDeploy === true, command.verbose === true, taskRoleName, terminationProtection, stackPolicy, cloudFormationRoleName, undefined, taskViaRoleArn);
